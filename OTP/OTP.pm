...
sub OTP {
	...
	for ( $i=0; $i<=$#message; $i++ ) {
		my $char = $message[$i];
		$cipher .= pack('C',unpack('C',$char)^unpack('C',$pad[$i]));
	}
	return $cipher;
}
...
Crypt::OTP - Perl implementation of the One Time Pad (hence, OTP) encryption method, the only
verifiably perfect form of encryption.

=head1 SYNOPSIS

  use Crypt::OTP;
  $cipher = OTP( $pad, $message );
	or
  $cipher = OTP( $pad, $message, $mode );
...
Kurt Kincaid, kurt.kincaid@neurogames.com
